	...
	public static void main(String[] args) {
		...
		for (int tan = 0; tan <= 9999; tan++) {
			...
			case 3:
				...
				System.out.println("1人目のポケを選べ");
				String bato1 = stdIn.next();
				System.out.println("２人目のポケを選べ");
				String bato2 = stdIn.next();

				for (int ku = 0; ku <= 5; ku++) {
					if(tre.myPockes[ku].name.equals("")) {
					}else if (tre.myPockes[ku].name.equals(bato1)) {
						for (int ku2 = 0; ku2 <= 5; ku2++) {
							if(tre.myPockes[ku2].name.equals("")) {
							}else if (tre.myPockes[ku2].name.equals(bato2)) {
								String kati = bto.doBattle(tre.myPockes[ku], tre.myPockes[ku2]);
